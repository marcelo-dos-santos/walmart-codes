  ...
  const handleFileUpload = async (uploadedRates: UploadedRecord[]) => {
    ...
    const progressInterval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        setStatusMessage("Your file is ready and has been automatically downloaded.");
  ...
  const formatDataForExcel = (data) => {
    ...
    return data.map((entry) => {
      ...
      if (selectedValues.department) {
        result["Department"] = findLabelByValue("department", selectedValues.department);
      }
      ...
      <div style={{ marginTop: "26%" }}>
        ...
        {isCalculating ? (
          ...
          >
            ...
            <Divider sx={{ my: 2 }} />
            <div style={{ display: "flex", justifyContent: "flex-end", width: "100%" }}>
              <ApplyButton onClick={handleUploadAnotherFile}>Try again</ApplyButton>
            </div>
          </Box>
        ) : uploadComplete ? (
          <Box
