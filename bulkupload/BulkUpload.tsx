...
export const BulkUpload = () => {
  ...
  const processFile = (file: File) => {
    if (ACCEPTED_FILE_TYPES.includes(file.type)) {
      ...
      reader.onload = (event: any) => {
        ...
        const processedDataForDisplay: any[] = [];
        const processedDataForUpload: {
          market_id: number | null;
          element_id: number | null;
          element_subtype_id: number | null;
          loading_port_id: number | null;
  ...
  const handleFileUpload = async (uploadedRates: UploadedRecord[]) => {
    ...
    try {
      ...
      const responseObjects: any[] | undefined = [];

      uploadedRates.forEach((rate: RateType, index: number) => {
    ...
    if (filterCriteria[subtypeId]) {
      try {
        ...
        if (response.data && response.data.data) {
          rates.push(
            ...response.data.data.map((rate: { [x: string]: any }) => ({
              ...rate,
      ...
      for (let i = 0; i < validIds.length; i++) {
        ...
        try {
          ...
          if (response.data && response.data.data) {
            const activeRates = response.data.data.filter((rate: any) =>
              isRateActive(rate)
            );
            rates.push(
              ...activeRates.map((rate: { [x: string]: any }) => ({
                ...rate,
                [subtypeId]: map.get(rate[subtypeId]) || rate[subtypeId],
              }))
            );
